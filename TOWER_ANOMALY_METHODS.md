...
- **Locally new Cell ID after a prior visit (active v2 method)**: the first reliable stationary visit builds the local baseline and never produces novelty evidence. Method `new_in_well_covered_place` can trigger only when the same coarse Cell ID (`operator + RAT + TAC/LAC + Cell ID`) was absent during at least three independent 10-minute operator/RAT windows in a completed prior visit, then repeats in at least two windows during a later visit. Visits are separated by more than six hours; midnight alone does not split one stay. Raw polling frequency, extra neighbor rows, and PCI/EARFCN variants do not increase the effect.
- **Stable dwell (active negative evidence)**: method `dwell_stability` treats longer unchanged observation as evidence of normality. It uses independent windows, visits, days, and detection rate with diminishing returns and a bounded contribution.
- **Unconfirmed novelty**: a single later-window sighting is displayed for review but does not score as locally new. Missing/zero Cell IDs, bad GPS, ignored observations, and moving observations cannot establish or trigger the local baseline.
- **Legacy raw-count methods (disabled)**: `ephemeral_stationary_opportunity`, `disappears_despite_coverage`, `new_area_code_in_well_covered_place`, and `place_change_correlation` remain available for historical comparison, but their raw-count/wall-clock outputs contribute zero score.
- **Transient Cell-ID activation v2 (shadow)**: `transient_activation_v2` requires at least two completed prior visits and 12 same-source opportunity windows, a fixed burst of at least four windows, and at least two later visits/12 later windows. A conservative Wilson-rate drop prevents raw polling density from becoming evidence.
- **Visit-aware disappearance v2 (shadow)**: `visit_aware_disappearance_v2` first establishes a repeatable Cell ID across visits, then requires at least two later visits and 12 later same-source windows with zero hits. A beta posterior-predictive probability must make that absence genuinely unlikely. A reappearance from any compatible source moves the cutoff forward.
- **Local TAC/LAC novelty v2 (shadow)**: `local_tac_novelty_v2` learns a source-specific local TAC/LAC pattern over at least two prior visits/12 windows. The previous dominant TAC/LAC must represent at least 85% of context, and the candidate must repeat in at least three windows and 60% of current opportunities. One representative carries a multi-cell TAC event.
- **Per-cell visit change v2 (shadow)**: `per_cell_visit_change_v2` compares fixed earlier and recent visits for the same source, signal metric, and EARFCN. It subtracts the median peer-cell change and requires repeated, robust residual movement. It does not search for the most favorable change point.

The v2 replacements run in **shadow mode**: candidates are visible in the map, tower table, drawer, database features, and method explanations, while their live score contribution remains exactly zero. This preserves the analytical functionality without silently trusting it before candidate review.
